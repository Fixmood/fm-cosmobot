...
askHandlers toolCfg tools cfg threads =
  ...
  , naturalHaltRoute threads
  , directVoiceRoute cfg
  , conversationRoute toolCfg tools cfg threads
  ]

-- Explicit voice requests bypass the general tool loop. Without this guard a
-- model can incorrectly build an MP3 in a sandbox and send it as a file card.
directVoiceRoute
  :: HandlerEffects es
  => AskHandlerConfig
  -> RouteHandler es
directVoiceRoute cfg =
  stopOn (matching \message ->
    (isAllowedPrivate message || (isAllowedGroup message && (message.digest.mentionsBot || calledByName cfg message)))
      && isVoiceRequest message.text) \message _ ->
    Concurrency.fire "ask.direct-voice" do
      let prompt = voicePrompt message.text
      AudioTools.generateAudioForMessage (FMBridge.fmStandaloneMessage message) prompt >>= \case
        Right _ -> pure ()
        Left err -> void (Chat.replyTo (FMBridge.fmStandaloneMessage message) (FMBridge.fmReplyBody [i|语音发送失败：#{err}|]))

isVoiceRequest :: Text -> Bool
isVoiceRequest raw =
  let input = Text.toCaseFold (Text.filter (not . (`elem` [' ', '\t', '\n', '\r', '\x3000'])) raw)
  in any (`Text.isInfixOf` input)
      [ "发一条语音"
      , "发语音"
      , "发个语音"
      , "说句语音"
...
calledByName :: AskHandlerConfig -> IncomingMessage -> Bool
calledByName cfg message =
  maybe False (\name -> case prefixedText name of MessageFilter matches -> isJust (matches message)) cfg.name

voicePrompt :: Text -> Text
voicePrompt raw =
  let cleaned = Text.strip raw
      markers = ["说：", "说:", "内容：", "内容:", "读：", "读:"]
...
isOtherReply _ = False

isTriggerManagementRequest :: AskHandlerConfig -> IncomingMessage -> Bool
isTriggerManagementRequest cfg message =
  (message.digest.senderIsSuperuser || message.digest.mentionsBot || calledByName cfg message)
    && any (`Text.isInfixOf` normalized) managementPhrases
  where
    normalized = Text.toCaseFold (Text.filter (not . (`elem` [' ', '\t', '\n', '\r', '\x3000'])) message.text)
    managementPhrases =
      [ "恢复默认触发"
      , "恢复这个群的默认触发"
      , "恢复这个房间的默认触发"
...
naturalHaltRoute
  ...
  -> RouteHandler es
naturalHaltRoute threads =
  stopOn naturalHaltFilter (handleHalt threads)

naturalHaltFilter :: MessageFilter Text
naturalHaltFilter = MessageFilter \message ->
  if message.digest.mentionsBot && naturalHaltIntent message.text
    then Just (if naturalAllHaltIntent message.text then "all" else "")
    else Nothing

naturalHaltIntent :: Text -> Bool
naturalHaltIntent text =
  let input = Text.toLower (Text.filter (not . (`elem` [' ', '\t', '\n', '\r', '\x3000'])) text)
  in any (`Text.isInfixOf` input)
      [ "停止当前任务", "停止agent", "停止任务", "停掉当前任务", "全部停止" ]

naturalAllHaltIntent :: Text -> Bool
naturalAllHaltIntent text =
  let input = Text.toLower (Text.filter (not . (`elem` [' ', '\t', '\n', '\r', '\x3000'])) text)
...
handleHalt threads message args
  ...
  | Text.null input = do
      ...
      case listToMaybe active of
        Nothing -> void $ Chat.replyTo message "当前没有可停止的任务。"
        Just thread -> do
          halted <- haltActiveThreadsForMessage threads Concurrency.cancel message [thread.id]
          void $ Chat.replyTo message (if null halted then "当前任务已经结束。" else "已停止当前任务。")
  | input == "all", isNothing message.replyToMessageId = do
      if isSuperuser message
        then do
          active <- listActiveThreadsForMessage threads message
          ...
          void $ Chat.replyTo message [i|已停止当前窗口的全部任务（#{length halted} 个）。|]
        else
          void $ Chat.replyTo message "只有 superuser 可以执行全部停止。"
  | otherwise =
...
startAskThread label toolCfg tools cfg threads resource message prompt = do
  ...
  let transcript = startWithIncomingMessage message input
...
startDrawThread label cfg threads message prompt = do
  ...
  -- Carry the id the platform returned, so the recorded row can be found again
...
fetchReferencedMessage
  :: (Chat.Chat :> es, ChatLog.ChatLog :> es, KatipE :> es)
  => IncomingMessage
...
fetchReferencedMessage message =
  case message.replyToMessageId of
    Nothing ->
      pure Nothing
    Just messageId ->
      ChatLog.lookupMessage message messageId >>= \case
        Just entry -> do
          logDebug [i|referenced message resolved from chat log: message=#{messageIdText messageId}|]
          pure (Just (chatLogEntryReferencedMessage entry))
        Nothing ->
          Chat.getMessageContent message messageId

chatLogEntryReferencedMessage :: ChatLog.ChatLogEntry -> ReferencedMessage
chatLogEntryReferencedMessage entry =
  ReferencedMessage
    { messageId = entry.messageId
    , senderDisplayName = entry.senderUsername
    , senderIdentifier = entry.senderId
    , senderIsBot = entry.isBot
    , text = entry.text
...
startThreadFromReply toolCfg tools cfg threads resource message parentId = do
  ...
  referenced <- fetchReferencedMessage message{replyToMessageId = Just parentId}
  let contextImages = maybe [] (.imageUrls) referenced <> message.imageUrls
  let contextFiles = referencedFiles referenced <> message.files
...
continueThread toolCfg tools cfg threads resource message parentKey transcript = do
  ...
  let nextTranscript =
        appendIncomingMessage message input (withoutLegacySystemPrompt transcript)
