...
instance ChatDriver driver => ChatDriver (NormalizingChatDriver driver) where
  ...
  sendMergedForward (NormalizingChatDriver driver) message nodes =
    withChatDriverEither message "merged forward reply" do
      sendMergedForward driver message nodes
...
instance ChatDriver ChatDrivers where
  ...
  sendReplyMessage drivers message body
    ...
    | otherwise = do
        ...
        recordRecentQQDeliveries drivers message body (rights [result])
        pure result
  ...
  sendReplyMessages drivers message body
    ...
    | otherwise = do
        ...
        recordRecentQQDeliveries drivers message body (rights results)
        pure results

  sendMergedForward drivers message nodes =
...
runChatDrivers
  :: (KatipE :> es, Concurrency.Concurrency :> es, HTTP.HTTP :> es, Timeout :> es, Process :> es, Fail :> es, Concurrent :> es, Media.Media :> es, FileSystem :> es, Prim :> es, Storage.Storage :> es, IOE :> es)
  => Maybe QQ.Config
...
bridgeIncomingMessages
  :: (ChatDriverEffects ChatDrivers es, Process :> es, IOE :> es)
  => ChatDrivers
  -> Stream (Of IncomingMessage) (Eff es) ()
  -> Stream (Of IncomingMessage) (Eff es) ()
bridgeIncomingMessages drivers =
...
bridgeIncomingMessage drivers message
  = do
      ...
      case takeoverRelay takeover message of
        Just (target, body) -> do
          result <- sendTakeoverMessage drivers target body
          unless (isRight result) $
        ...
        Nothing
          ...
          | Just bridged <- FMBridge.matrixOwnerAsQQ message -> do
              case drivers.qq of
                ...
                Just qqDriver -> do
                  ...
                  results <- sendReplyMessages (NormalizingChatDriver qqDriver) target
                    (FMBridge.fmOwnerRelayBodyWithImages message.text message.imageUrls)
          ...
          | otherwise ->
              pure (Just message)
  where
    takeoverRelay (Just state) current
      | FMBridge.takeoverEnabled state
      , current.eventKind == IncomingMessageCreated
      , FMBridge.takeoverMatchesDestination state current
      , FMBridge.takeoverIsOwnerMessage current
      , not (FMBridge.takeoverIsControlMessage current.text) =
          Just (FMBridge.takeoverTargetMessage (FMBridge.takeoverSource state),
            FMBridge.takeoverOwnerBodyWithImages current.text current.imageUrls)
      | FMBridge.takeoverEnabled state
      , current.eventKind == IncomingMessageCreated
      , FMBridge.takeoverMatchesSource state current
      , not (FMBridge.takeoverIsBotMessage current) =
          Just (FMBridge.takeoverTargetMessage (FMBridge.takeoverDestination state),
            FMBridge.takeoverIncomingBodyWithImages current)
    takeoverRelay _ _ = Nothing
...
sendTakeoverMessage
  ...
  -> Text
  -> Eff es (Either Text MessageId)
sendTakeoverMessage drivers target body =
  withMessageDriver drivers target \driver ->
    sendReplyMessage (NormalizingChatDriver driver) target body
