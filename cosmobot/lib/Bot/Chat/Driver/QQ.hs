...
instance Driver.ChatDriver QQDriver where
  type ChatDriverEffects QQDriver es = (IOE :> es, KatipE :> es, Timeout :> es, Concurrent :> es, Process :> es, Concurrency.Concurrency :> es, FileSystem.FileSystem :> es, Media.Media :> es)
...
incomingMessages driver = do
  ...
  case eventToIncomingMessageWith driver.config event of
    ...
    Just parsedMessage -> do
      normalized <- S.lift (normalizeQQMessageFiles parsedMessage)
      message <- S.lift (expandIncomingForwardedMessage driver normalized)
...
replyToQQ driver message body =
  case (message.kind, message.chatId, message.senderId) of
    (ChatGroup, Just groupId, _) -> do
      if length (Chat.replyImageUrls body) >= 2
        then sendMergedForwardQQ driver message [automaticImageForwardNode driver body]
        else do
...
-- Keep multi-image replies inside one QQ merged-forward message.  A single
-- node preserves the original text and all images without creating a burst
-- of ordinary group messages.
automaticImageForwardNode
  :: QQDriver
  -> Text
  -> Aeson.Value
automaticImageForwardNode driver body =
  Aeson.object
    [ "type" Aeson..= Aeson.String "node"
    , "data" Aeson..= Aeson.object
        [ "name" Aeson..= Aeson.String "FM"
        , "uin" Aeson..= Text.pack (show (fromMaybe 0 driver.config.botQQ :: Integer))
  ...
  where
    textContent =
      [ Aeson.object
          ...
          , "data" Aeson..= Aeson.object ["text" Aeson..= Chat.renderReplyBody body]
          ]
      | not (Text.null (Text.strip (Chat.renderReplyBody body)))
      ]
    imageContent =
      [ Aeson.object
      ...
      | imageUrl <- Chat.replyImageUrls body
      ]
...
sendMergedForwardQQ
  ...
  -> [Aeson.Value]
  -> Eff es (Either Text MessageId)
sendMergedForwardQQ driver message nodes =
  case (message.kind, message.chatId) of
    (ChatGroup, Just groupId)
      | not (null nodes) -> do
          normalizedNodes <- traverse (normalizeQQForwardNode driver) nodes
          response <- sendAction driver (Aeson.object
            [ "action" Aeson..= Aeson.String "send_group_forward_msg"
            , "params" Aeson..= Aeson.object
                [ "group_id" Aeson..= groupId
                , "messages" Aeson..= normalizedNodes
                ]
            ])
          qqMessageIdResult "send_group_forward_msg" response
      | otherwise -> pure (Left "QQ merged forward requires at least one message node.")
    _ -> pure (Left "QQ merged forward is currently available for group messages only.")

normalizeQQForwardNode
  :: (IOE :> es, FileSystem.FileSystem :> es, Media.Media :> es)
  => QQDriver
  -> Aeson.Value
  -> Eff es Aeson.Value
normalizeQQForwardNode driver = \case
  Aeson.Array values ->
    Aeson.Array . fromList <$> traverse (normalizeQQForwardNode driver) (toList values)
  Aeson.Object object -> do
    normalized <- traverse (normalizeQQForwardNode driver) object
    case (KeyMap.lookup "type" normalized, KeyMap.lookup "data" normalized) of
      (Just (Aeson.String "image"), Just (Aeson.Object imageData)) -> do
        let fileRef = KeyMap.lookup "file" imageData >>= valueText
        case fileRef of
          Nothing -> pure (Aeson.Object normalized)
          Just ref -> do
            file <- qqImageFile ref
            pure (Aeson.Object (KeyMap.insert "data" (Aeson.Object (KeyMap.insert "file" (Aeson.String file) imageData)) normalized))
      _ -> pure (Aeson.Object normalized)
  value -> pure value
  where
    valueText (Aeson.String value) = Just value
    valueText _ = Nothing
...
uploadFileQQ
  :: (IOE :> es, KatipE :> es, Timeout :> es, Concurrent :> es, Process :> es, FileSystem.FileSystem :> es)
  => QQDriver
  -> IncomingMessage
  -> FilePath
  -> Maybe Text
...
replyAudioQQ driver message audioRef caption =
  case (message.kind, message.chatId, message.senderId) of
    (ChatGroup, Just groupId, _) -> do
      preparedRef <- prepareQQAudio audioRef
      qqMessage <- audioMessage preparedRef caption
      response <- sendAudioMessage driver "send_group_msg"
        [ "group_id" Aeson..= groupId
        , "message" Aeson..= qqMessage
        ]
      audioResponseOrFallback preparedRef response "send_group_msg"
    (ChatPrivate, _, Just rawUserId)
      | Just userId <- parseIntegerUserId rawUserId -> do
      preparedRef <- prepareQQAudio audioRef
      qqMessage <- audioMessage preparedRef caption
      ...
      audioResponseOrFallback preparedRef response "send_private_msg"
    _ ->
      pure (Left "QQ audio reply requires a QQ group or private chat with a known target.")
  where
    audioResponseOrFallback preparedRef response action
      | actionSucceeded response =
          qqMessageIdResult action response
      | Just path <- localAudioPath preparedRef =
...
prepareQQAudio ref =
  case localAudioPath ref of
    Nothing -> pure ref
    Just source -> do
      let target = source <> ".qq.ogg"
      result <- readCreateProcessWithExitCode
        (proc "ffmpeg" ["-y", "-i", source, "-vn", "-c:a", "libopus", "-b:a", "32k", target])
        ""
      pure case result of
        (Exit.ExitSuccess, _, _) -> "file://" <> Text.pack target
        _ -> ref
...
deleteMessageQQ driver _ messageId =
  case messageIdInteger messageId of
    Just rawMessageId -> do
      ...
      let succeeded = actionSucceeded response
      unless succeeded $
        logWarning [i|#{qqUploadFailureText "delete_msg" response} message_id=#{rawMessageId}|]
      pure succeeded
    _ ->
      pure False
...
-- | Expand a directly received QQ merged-forward message before it reaches
-- the agent. Reply lookups use the same expansion path below, but a direct
-- forward arrives as a normal message event and otherwise only exposes the
-- placeholder card to the model.
expandIncomingForwardedMessage
  :: (IOE :> es, KatipE :> es, Timeout :> es, Concurrent :> es)
  => QQDriver
  -> IncomingMessage
  -> Eff es IncomingMessage
expandIncomingForwardedMessage driver message = do
  let sources = incomingForwardSources message.raw
  if null sources
    then pure message
    else do
      forwarded <- foldMapM (expandForwardedSource driver Set.empty) sources
      let updatedMessage :: IncomingMessage
...
incomingForwardSources :: Aeson.Value -> [ForwardedSource]
incomingForwardSources raw =
  maybe [] messageForwardSources (join (Aeson.parseMaybe messageField raw))
  where
    messageField = Aeson.withObject "QQ incoming message" (Aeson..:? "message")
...
eventToIncomingMessageWith cfg event
  ...
  | isPokeEvent cfg event =
      Just IncomingMessage
        ...
        , chatId = event.groupId <|> event.userId
        , chatAliases = []
        , digest = (qqMessageDigest cfg event){mentionsBot = True}
        , senderId = show <$> event.userId
        , senderUsername = event.sender >>= qqSenderDisplayName
        , messageId = Nothing
        , replyToMessageId = Nothing
        , mentions = maybe [] (pure . show) (event.selfId <|> cfg.botQQ)
        , mentionUsernames = []
        , imageUrls = []
        , files = []
        , text = "[QQ戳一戳事件] 对方刚刚戳了你一下。结合当前聊天自然回应，不要复用固定句式，也不要解释这是系统提示。"
        , raw = event.rawEvent
        }
...
isPokeEvent :: Config -> Event -> Bool
isPokeEvent cfg event =
  event.postType == "notice"
    && isJust do
      expectedBotId <- event.selfId <|> cfg.botQQ
      targetId <- Aeson.parseMaybe parseTarget event.rawEvent
      guard (targetId == expectedBotId)
  where
    parseTarget = Aeson.withObject "QQ poke event" \o -> do
      noticeType <- o Aeson..: "notice_type"
      subType <- o Aeson..: "sub_type"
      guard (noticeType == ("notify" :: Text) && subType == ("poke" :: Text))
      o Aeson..: "target_id"
...
qqMessageDigest cfg event =
  ...
  where
    ...
    isPrivateMessage =
      event.postType == "message" && event.messageType == Just "private" && isJust event.userId
    senderSuperuser =
